...
!(function () {
  ...
  function createElement(tag, classes, parent, html, title) {
    ...
    for (let i = classes.length; i--; el.classList.add(classes[i]));
  ...
  function DPX(input) {
    var options = {};
            ...
            var inited   = false;
            var value    = null;
            var mode = 2; // 2 - days, 1 - months, 0 - years
    ...
    function getFormatedDate(dt, format) {
      ...
      return format.replace(/([dmM]{1,2}|D|yyyy|yy)/g, (match)
            => {
    ...
    function draw(dt) {
      ...
      var setMonth = dt.getMonth();
                ...
                var zeroYear = Math.floor(setYear / 10) * 10;
      ...
      var getter = ['getFullYear', 'getMonth', 'getDate'][mode];
                var setter = ['setFullYear', 'setMonth', 'setDate'][mode];
                var i = mode === 2 ? 42 : 16;
      for (; i--; dt[setter](dt[getter]() + 1)) {
        var classes = ['dpx-item'];
                    var title = getFormatedDate(dt, [options.titleFormatYear, options.titleFormatMonth, options.titleFormatDay][mode]);
        ...
        var content = mode ? (mode === 2 ? dt.getDate() : options.shortMonthLabels[dt.getMonth()]) : dt.getFullYear();
                    var el = createElement('span', classes, elements.content, content, title);
